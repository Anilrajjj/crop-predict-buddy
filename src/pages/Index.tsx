...
import { Leaf, ArrowRight, BarChart3, Droplets, Zap } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import AgricultureForm from "@/components/AgricultureForm";
import ResultsDashboard from "@/components/ResultsDashboard";
import { predictAgricultureRecommendations } from "@/utils/mlModel";
...
const Index = () => {
  ...
  const handleFormSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      const predictions = await predictAgricultureRecommendations(inputData);
      setResults(predictions);
      
      toast({
        title: "Analysis Complete!",
        description: `Generated recommendations for ${formData.cropType} with ${predictions.confidence}% confidence.`,
      });
        ...
        <div className="relative z-10 container mx-auto px-4 py-20 lg:py-32">
          <div className="max-w-4xl mx-auto text-center text-white">
            <div className="flex items-center justify-center gap-3 mb-6">
              <Leaf className="w-12 h-12 text-harvest-gold" />
              <h1 className="text-5xl lg:text-7xl font-bold">
                Smart Agriculture
              </h1>
            </div>
            <p className="text-xl lg:text-2xl mb-8 text-white/90 leading-relaxed max-w-3xl mx-auto">
              Harness the power of AI to optimize your crop yields with precision irrigation 
              and fertilization recommendations tailored to your soil and weather conditions.
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              <Card className="bg-white/10 backdrop-blur-sm border-white/20 text-white">
                <CardContent className="p-6 text-center">
                  <Droplets className="w-8 h-8 text-sky-blue mx-auto mb-3" />
                  <h3 className="font-semibold mb-2">Smart Irrigation</h3>
                  <p className="text-sm text-white/80">AI-powered water optimization</p>
                </CardContent>
              </Card>
              
              <Card className="bg-white/10 backdrop-blur-sm border-white/20 text-white">
                <CardContent className="p-6 text-center">
                  <Zap className="w-8 h-8 text-harvest-gold mx-auto mb-3" />
                  <h3 className="font-semibold mb-2">Precision Fertilization</h3>
                  <p className="text-sm text-white/80">Optimized nutrient management</p>
                </CardContent>
              </Card>
              
              <Card className="bg-white/10 backdrop-blur-sm border-white/20 text-white">
                <CardContent className="p-6 text-center">
                  <BarChart3 className="w-8 h-8 text-leaf-green mx-auto mb-3" />
                  <h3 className="font-semibold mb-2">Yield Prediction</h3>
                  <p className="text-sm text-white/80">Data-driven crop forecasting</p>
                </CardContent>
              </Card>
            </div>
          </div>
        ...
        {!results ? (
          <>
            ...
            <div className="max-w-3xl mx-auto text-center mb-12">
              ...
              <p className="text-lg text-muted-foreground leading-relaxed">
                Our advanced machine learning model analyzes your soil composition, local weather patterns, 
                and crop type to provide precise irrigation and fertilization recommendations that can 
                increase your yield by up to 45%.
              </p>
            </div>
          ...
          <>
            ...
            <div id="results-section" className="space-y-8">
              ...
              <ResultsDashboard results={results} cropType={currentCrop} />
            </div>
          </>
        )}
      </section>
